...
INSERT INTO @Alerts
SELECT 'Database Settings', 'Database Auto Close is ON', name, 'Strongly recommended to set Database Auto Close to OFF'
FROM sys.databases
WHERE state_desc = 'ONLINE'
...
INSERT INTO @Alerts
SELECT 'Database Settings', 'File Auto Growth is Disabled', 'File ' + QUOTENAME(name) + ' in database ' + QUOTENAME(DB_NAME(database_id)) + ' has Auto Growth disabled'
...
INSERT INTO @Alerts
SELECT 'Database Settings', N'Database File(s) on Volume C'
...
SELECT 'Performance', 'Not recommended instance configuration', 'SQL Server Max Memory', Report
FROM
(
...
SELECT R.category, 'Not recommended instance configuration', R.setting
, R.errormsg
+ N' (current value: ' + CONVERT(nvarchar, c.[value]) + N', recommended value: ' + CONVERT(nvarchar,R.recommendedvalue) + ')'
FROM sys.configurations AS c
CROSS APPLY
(
SELECT 'Security', 'allow updates', 0, 'Direct System Catalog Updates is enabled'
UNION ALL SELECT 'Performance', 'automatic soft-NUMA disabled', 0, 'Auto Soft NUMA is disabled'
UNION ALL SELECT 'Performance', 'awe enabled', CASE WHEN @sqlmajorver < 11 AND @@VERSION NOT LIKE '%<X64>%' AND @@VERSION NOT LIKE '%<IA64>%' AND @systemmem >= 4000 THEN 1 WHEN @sqlmajorver > 10 THEN 0 END, 'AWE setting is not optimal for this instance'
UNION ALL SELECT 'Performance', 'backup compression default', 1, 'Backup Compression by default is not enabled'
UNION ALL SELECT 'Automation', 'default trace enabled', 1, 'Default trace setting is NOT enabled'
UNION ALL SELECT 'Performance', 'lightweight pooling', 0, 'Lightweight pooling setting is not the recommended value'
UNION ALL SELECT 'Performance', 'network packet size (B)', 4096, 'Network packet size is not the default value'
) AS R (category, setting, recommendedvalue, errormsg)
WHERE c.name = R.setting
AND CONVERT(int, c.[value]) <> R.recommendedvalue
 
UNION ALL
 
SELECT 'Performance', 'Not recommended instance configuration', 'Affinity Mask and Affinity I/O Mask',
...
SELECT 'Performance', 'Not recommended instance configuration', 'Blocked Process Threshold'
...
SELECT 'Performance', 'Not recommended instance configuration', 'min memory per query (KB) and index create memory (KB)'
...
INSERT INTO @Alerts
SELECT 'Database Settings', 'Invalid database owner', QUOTENAME(db.name)
...
INSERT INTO @Alerts
SELECT 'Database Settings', 'DB Page Verification different from CHECKSUM', QUOTENAME(name), 'Current setting: ' + page_verify_option_desc
FROM sys.databases
...
INSERT INTO @Alerts
SELECT 'Instance Settings', N'@@SERVERNAME different from actual server name', N'@@SERVERNAME: ' + @@SERVERNAME, N'Actual server name: ' + CONVERT(nvarchar,SERVERPROPERTY('ServerName'))
...
EXEC sp_MSforeachdb '
IF EXISTS (SELECT * FROM sys.databases WHERE state_desc = ''ONLINE'' AND name = ''?'')
SELECT ''Data Integrity'', ''Untrusted Check Constraint(s)'', ''?'', QUOTENAME(OBJECT_SCHEMA_NAME(parent_object_id, DB_ID(''?''))) + ''.'' + QUOTENAME(OBJECT_NAME(parent_object_id, DB_ID(''?''))) + ''.'' + QUOTENAME(name)
...
EXEC sp_MSforeachdb '
IF EXISTS (SELECT * FROM sys.databases WHERE state_desc = ''ONLINE'' AND name = ''?'')
SELECT ''Data Integrity'', ''Untrusted Foreign Key(s)'', ''?'', QUOTENAME(OBJECT_SCHEMA_NAME(parent_object_id, DB_ID(''?''))) + ''.'' + QUOTENAME(OBJECT_NAME(parent_object_id, DB_ID(''?''))) + ''.'' + QUOTENAME(name)
FROM [?].sys.foreign_keys
WHERE is_not_trusted = 1 AND is_not_for_replication = 0 AND is_disabled = 0'


INSERT INTO @Alerts
select 'Database Settings', 'Database is not online!', QUOTENAME(db.name),  'Database status is ' + db.state_desc
from sys.databases db
...
INSERT INTO @Alerts
SELECT 'Data Integrity', 'Identity Overflow Alert', QUOTENAME(DatabaseName)
